...
// ═══════════════════════════════════════════════════════════════════
// MIGRASI: @whiskeysockets/baileys → shiraori-baileys
//
// shiraori-baileys adalah wrapper di atas @whiskeysockets/baileys
// yang menambahkan: FastMode, extractPhoneNumber (LID resolver),
// AntiBadSession, PairingCode manager, dll.
//
// Semua import Baileys di-proxy melalui shiraori-baileys untuk
// memastikan konsistensi. @whiskeysockets/baileys tetap ada di
// node_modules karena shiraori-baileys membutuhkannya sebagai peer.
// ═══════════════════════════════════════════════════════════════════
...
// ── Nomor target pairing diambil dari .env (PAIRING_NUMBER) ───────
// Normalisasi: 0812xxxx -> 62812xxxx (asumsi default Indonesia)
...
let pairingNumber = normalizePairingNumber(process.env.PAIRING_NUMBER)
...
async function createSocket(version) {
    ...
    if (Browsers && typeof Browsers.ubuntu === 'function') {
        ...
        browserConfig = ['Ubuntu', 'Chrome', '110.0.0']
    }
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            ...
            } else if (!useMobile && !globalState.creds.registered && !pairingNumber) {
                console.log('\x1b[31m[PAIRING]\x1b[0m PAIRING_NUMBER tidak ada di .env — tidak bisa minta pairing code.')
                console.log('\x1b[33m[PAIRING]\x1b[0m Set PAIRING_NUMBER di .env lalu restart bot.')
            }
            // QR dinonaktifkan total — bot hanya pairing lewat kode, tidak pernah tampilkan QR.
        }
...
;(async () => {
    try {
        ...
        // STEP 1.5: Session belum tertaut → pairing code pakai nomor dari .env
        // (koneksi selalu lewat pairing code, QR dinonaktifkan total)
        if (!useMobile && !globalState.creds.registered) {
            if (!pairingNumber) {
                console.log('')
                console.log('\x1b[31m[PAIRING]\x1b[0m PAIRING_NUMBER belum di-set atau tidak valid di .env')
                console.log('\x1b[33m[PAIRING]\x1b[0m Tambahkan baris berikut di file .env lalu jalankan ulang bot:')
                console.log('\x1b[36m%s\x1b[0m', '  PAIRING_NUMBER=62895803135347   (contoh, pakai nomor WA bot kamu)')
                console.log('')
                throw new Error('PAIRING_NUMBER tidak ditemukan di .env')
            }
            console.log('')
            console.log('\x1b[32m[PAIRING]\x1b[0m Nomor target dari .env: +' + pairingNumber)
            console.log('')
        }
