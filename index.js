...
module.exports = {
    async handler(chatUpdate) {
        ...
        // Disabled verbose logging for clean console
        // if (chatUpdate.messages.length > 1) console.log(chatUpdate.messages)
        let m = chatUpdate.messages[chatUpdate.messages.length - 1]
        if (!m) return
        // console.log(JSON.stringify(m, null, 4)) // Disabled - too verbose
        
        // Only log simple message info
        try {
        ...
        if (settings) {
          ...
          status: 0,
            ...
            // Fixed owner checker - handle both with and without @s.whatsapp.net
            let senderNumber = m.sender.replace(/[^0-9]/g, '')
            let isROwner = [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '')).includes(senderNumber) || 
                          [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            let isOwner = isROwner || m.fromMe
            let isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '')).includes(senderNumber) || 
                        global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            let isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '')).includes(senderNumber) || 
