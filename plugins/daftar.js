...
let handler = async function (m, { conn, text, usedPrefix, command }) {
    let _key = jidToNum(m.sender)

    // Coba resolve ke nomor WA sebenarnya via lidMap jika panjang dan tidak umum
    // (misal 244912071147724)
    if (_key && _key.length > 14) {
        let _resolved = null

        // 1. Cari di lidMap: value == _key (nomor WA dari LID ini)
        try {
            const lidMap = global.db?.data?.settings?.lidMap || {}
            // Cek apakah _key itu sendiri adalah LID number yang sudah punya mapping
            const _lidJid = _key + '@lid'
            if (lidMap[_lidJid]) {
                _resolved = lidMap[_lidJid]
            }
            // Atau cari LID yang valuenya sudah cocok
            if (!_resolved) {
                for (const [k, v] of Object.entries(lidMap)) {
                    if (v === _key && k.endsWith('@lid')) { _resolved = v; break }
                }
            }
        } catch (_) {}

        // 2. Coba dari conn.resolveLid
        if (!_resolved) {
            try {
                const _r = conn.resolveLid?.(_key + '@lid', m.chat)
                if (_r && !_r.endsWith('@lid')) {
                    _resolved = _r.includes('@') ? _r.split('@')[0].split(':')[0] : _r
                }
            } catch (_) {}
        }

        // 3. Cari di conn.contacts
        if (!_resolved) {
            try {
                const _cLid = _key + '@lid'
                const _c = conn.contacts?.[_cLid]
                if (_c?.id && _c.id.endsWith('@s.whatsapp.net')) {
                    _resolved = _c.id.split('@')[0].split(':')[0]
                }
            } catch (_) {}
        }

        if (_resolved && /^\d{5,20}$/.test(_resolved)) {
            // Resolve berhasil — update m.sender juga biar plugin lain konsisten
            _key = _resolved
            m.sender = _resolved
            // Simpan ke lidMap
            try {
                if (!global.db.data.settings.lidMap) global.db.data.settings.lidMap = {}
                global.db.data.settings.lidMap[_key.replace(/^(\d+)$/, '$1@lid')] = _resolved
                global.db.write().catch(() => {})
            } catch (_) {}
        } else {
            // Tolak pendaftaran jika masih berupa LID agar tidak mengotori database
            throw `❌ Pendaftaran gagal karena nomor WA kamu disembunyikan oleh WhatsApp (Community Group Privacy).\n\nBot tidak dapat mendeteksi nomor aslimu dan hanya menerima ID rahasia: \`${_key}\`\n\n*Solusi:*\n1. Coba daftarkan diri kamu melalui Pesan Pribadi (PM/Japri) ke bot ini.\n2. ATAU minta owner bot mengetik:\n\`.setlid <nomor_wa_kamu> ${_key}\`\nLalu coba daftar lagi di grup ini.`
        }
    }

    if (!_key) throw '❌ Gagal mendeteksi nomor WA. Coba kirim pesan lagi.'
    ...
    if (!user || user._temp) {
        global.db.data.users[_key] = {
            ...
            countryRole: 'warga', wanted: 0, criminalRecord: 0
        }
    ...
    if (!user.nik) user.nik = generateNik()
    if (!user.registeredAt) user.registeredAt = new Date(user.regTime).toISOString()

    // FIX: invalidate user index cache so handler.js doesn't serve stale "unregistered" state
    if (typeof global._invalidateUserIndex === 'function') global._invalidateUserIndex()
