...
let handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
    ...
    switch (type) {
        ...
        case 'welcome':
            if (m.isGroup) {
                // Admin/owner di grup → toggle hanya grup ini
                if (!(isAdmin || isOwner)) return global.dfail('admin', m, conn)
                chat.welcome = isEnable
                global.db.data.chats[m.chat] = chat
                break
            }
            // Owner via DM → toggle SEMUA chat sekaligus
            if (!isOwner) return global.dfail('owner', m, conn)
            isAll = true
            for (const chatId of Object.keys(global.db.data.chats)) {
                global.db.data.chats[chatId].welcome = isEnable
            }
            setting.welcome        = isEnable
            global.opts['welcome'] = isEnable
            break
        ...
        case 'antitoxic':
            ...
            isAll = true
            for (const chatId of Object.keys(global.db.data.chats)) {
                global.db.data.chats[chatId].antiToxic = isEnable
            }
            setting.antiToxic        = isEnable
            global.opts['antiToxic'] = isEnable
            break
...
╭─「 Daftar Opsi 」
│${isOwner ? '\n├ 🌐 public / grouponly\n├ 🌐 welcome (global via DM owner)\n├ 🌐 antitoxic (global via DM owner)\n├ 🚫 anticall / antispam\n├ 🚫 antitroli / antilink\n├ 🔧 autoread / restrict\n├ 🔧 backup / jadibot / nsfw\n├ 🔇 simi / autolevelup' : ''}
├ 💬 welcome (per-chat) / delete
├ 💬 antispam / antilink
├ 💬 simi / antitoxic
│
├ Contoh:
...
`.trim()
    ...
    // Simpan settings + write untuk semua case selain 'bot'/'aktif'
    global.db.data.settings = setting
    await global.db.write()

    const totalChats = isAll ? Object.keys(global.db.data.chats).length : null
    const scope = isAll
        ? `untuk *${totalChats}* chat (global)`
        : isUser
            ? 'untuk kamu'
            : 'untuk chat ini'

    m.reply(`✅ *${type}* berhasil di *${isEnable ? 'aktifkan' : 'nonaktifkan'}* ${scope}`)
}
