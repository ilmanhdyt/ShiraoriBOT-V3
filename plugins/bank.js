...
let handler = async (m, { conn, command, args, usedPrefix, participants = [] }) => {
    ...
    const user   = global.db.data.users?.[jidToNum(sender)]
    ...
    if ((user.bank || 0) > 0) {
        ...
        if (!user.lastTax) { user.lastTax = now }
            ...
            if (elapsed >= 0.01) {
                taxPotong    = Math.floor(user.bank * 0.02 * elapsed)
                user.bank    = Math.max(0, user.bank - taxPotong)
                user.lastTax = now
            }
        }
    }
    if (interest > 0 || taxPotong > 0) await saveDB()
    ...
    if (/^bank$/.test(cmd) && !args[0]) {
        ...
        const bankLine = user.bank > 0
            ? `│  🏦 *Bank   :* ${fmt(user.bank)} rupiah${taxPotong > 0 ? ` _(pajak -${fmt(taxPotong)})_` : ''}`
            : `│  🏦 *Bank   :* 0 rupiah`
        ...
        const bodyText = [
            ...
            bankLine,
            user.bank > 0 ? '│  🕐 *Pajak bank:* 1%/jam' : null,
        ...
        const buttons = [
            ...
            { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: '💵 Dompet',    id: `${usedPrefix}dompet` }) },
        ...
        try {
                    ...
                    viewOnceMessage: {
                        message: {
                            ...
                            interactiveMessage: proto.Message.InteractiveMessage.create({
                                header: { title: '🏦 Bank Negara', hasMediaAttachment: false },
                                body  : { text: bodyText },
                                footer: { text: wm },
            ...
            console.error('[bank] gagal kirim interactive:', e.message)
